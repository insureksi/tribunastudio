 ...
 * - $bookings                 : array daftar booking.
 * - $studios                  : array daftar studio (id, name) untuk filter.
 * - $stats                    : array statistik inline (optional).
 * - $widget_stats             : array statistik global untuk widget (optional).
 * - $upcoming_bookings_admin  : array upcoming bookings (next 7 days) untuk widget.
 * - $payment_window_seconds   : int, durasi batas pembayaran (detik).
 */
...
// Build export URL (admin-post) dengan filter yang sama.
$export_args = array(
...
// Widget stats defaults (global summary).
...
// Upcoming bookings list defaults untuk widget.
$upcoming_bookings_admin = isset( $upcoming_bookings_admin ) && is_array( $upcoming_bookings_admin )
    ? $upcoming_bookings_admin
    : array();

// Payment window default (fallback 0 = timer tidak aktif).
$payment_window_seconds = isset( $payment_window_seconds ) ? (int) $payment_window_seconds : 0;

// Helper untuk label tanggal + jam di widget upcoming.
...
<div class="wrap tsrb-bookings">
    ...
    <?php
    // BLOK 0: Global overview widgets.
    ...
    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="tsrb-bookings-bulk-form">
        ...
        <div class="tsrb-admin-table-wrap tsrb-admin-table-wrap-responsive tsrb-bookings-table-wrapper">
            <table class="widefat fixed striped tsrb-bookings-table tsrb-user-bookings-table"
                ...
                <thead>
                    <tr>
                        <td id="cb" class="manage-column column-cb check-column tsrb-col-checkbox">
                        ...
                        <th class="tsrb-col-customer"><?php esc_html_e( 'Customer', 'tribuna-studio-rent-booking' ); ?></th>
                        ...
                        <th class="tsrb-col-status tsrb-nowrap"><?php tsrb_admin_sort_link( 'status', __( 'Status', 'tribuna-studio-rent-booking' ), $current_orderby, $current_order ); ?></th>
                        ...
                        <th class="tsrb-col-timer tsrb-nowrap"><?php esc_html_e( 'Payment timer', 'tribuna-studio-rent-booking' ); ?></th>
                        <th class="tsrb-col-actions"><?php esc_html_e( 'Actions', 'tribuna-studio-rent-booking' ); ?></th>
                    </tr>
                </thead>
                <tbody>
                <?php if ( ! empty( $bookings ) ) : ?>
                    <?php foreach ( $bookings as $booking ) : ?>
                        ...
                        <tr class="<?php echo esc_attr( $row_class ); ?>">
                            <th scope="row" class="check-column tsrb-col-checkbox">
                                <input type="checkbox" name="booking_ids[]" value="<?php echo esc_attr( $booking->id ); ?>" class="tsrb-booking-checkbox" />
                            </th>
                            <td class="tsrb-col-id"><?php echo esc_html( $booking->id ); ?></td>
                            ...
                            <td class="tsrb-col-date tsrb-nowrap">
                                ...
                                if ( ! empty( $booking->date ) ) {
                                    ...
                                    echo esc_html( $date_ts ? date_i18n( 'd-m-Y', $date_ts ) : $booking->date );
                                } else {
                                    echo '&mdash;';
                                }
                                ?>
                            ...
                            <td class="tsrb-col-status tsrb-status-cell">
                                <div class="tsrb-booking-status-inline-wrapper">
                                    <select name="status" class="tsrb-booking-status-select" data-booking-id="<?php echo esc_attr( $booking->id ); ?>">
                            ...
                            <td class="tsrb-col-created">
                                ...
                                if ( ! empty( $booking->created_at ) ) {
                                    ...
                                    if ( $created_ts ) {
                                        $created_date = date_i18n( 'd-m-Y', $created_ts );
                                        $created_time = date_i18n( 'H:i', $created_ts );
                                        echo '<div class="tsrb-created-date">' . esc_html( $created_date ) . '</div>';
                                        echo '<div class="tsrb-created-time">' . esc_html( $created_time ) . '</div>';
                                    } else {
                            ...
                            <td class="tsrb-col-actions tsrb-actions-column">
