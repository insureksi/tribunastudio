...
/** @var array $settings */
/*
 * Gunakan option baru 'tsrbsettings' sebagai sumber utama.
 * Tetap baca dari 'tsrb_settings' jika 'tsrbsettings' belum ada (backward compatible).
 */
...
$timezone       = isset( $settings['timezone'] ) ? $settings['timezone'] : 'Asia/Jakarta';
...
$operating_hours = isset( $settings['operating_hours'] ) && is_array( $settings['operating_hours'] )
...
$integrations_defaults = array(
	...
	'whatsapp_default_message' => __(
		'Hi, I would like to ask about my booking ({booking_id}) on {booking_date}.',
		'tribuna-studio-rent-booking'
	),
);
...
$current_tab = isset( $_GET['tab'] ) && isset( $tabs[ $_GET['tab'] ] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'general';
...
foreach ( $expected_tables as $t ) {
	$exists              = ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $t ) ) === $t ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
	$tables_status[ $t ] = $exists;
}
...
$cron_event = function_exists( 'wp_get_scheduled_event' ) ? wp_get_scheduled_event( 'tsrb_auto_cancel_unpaid' ) : null;
?>
<div class="wrap tsrb-settings-page">
	...
	<form method="post" action="options.php">
		<?php
		// Registrasi group tetap memakai tsrb_settings_group,
		// tapi semua field name mengarah ke 'tsrbsettings[...]' agar tersimpan di option baru.
		settings_fields( 'tsrb_settings_group' );
		do_settings_sections( 'tsrb_settings_group' );
		?>
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						...
						<td>
							<input type="text" id="tsrb_currency" name="tsrbsettings[currency]" value="<?php echo esc_attr( $currency ); ?>" class="regular-text" />
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<input type="email" id="tsrb_admin_email" name="tsrbsettings[admin_email]" value="<?php echo esc_attr( $admin_email ); ?>" class="regular-text" />
						</td>
					</tr>

					<tr>
						<th scope="row">
							<label for="tsrb_timezone"><?php esc_html_e( 'Timezone', 'tribuna-studio-rent-booking' ); ?></label>
						</th>
						<td>
							<input type="text" id="tsrb_timezone" name="tsrbsettings[timezone]" value="<?php echo esc_attr( $timezone ); ?>" class="regular-text" />
							<p class="description"><?php esc_html_e( 'Example: Asia/Jakarta', 'tribuna-studio-rent-booking' ); ?></p>
						</td>
					</tr>

					<tr>
						<th scope="row">
							<label for="tsrb_admin_whatsapp_number"><?php esc_html_e( 'Admin WhatsApp Number', 'tribuna-studio-rent-booking' ); ?></label>
						</th>
						<td>
							<input type="text"
								   id="tsrb_admin_whatsapp_number"
								   name="tsrbsettings[admin_whatsapp_number]"
					...
					<tr>
						<th scope="row">
							<label><?php esc_html_e( 'Payment QR Code', 'tribuna-studio-rent-booking' ); ?></label>
						</th>
						<td>
							<div id="tsrb-payment-qr-wrapper">
								<input type="hidden" id="tsrb_payment_qr_image_id" name="tsrbsettings[payment_qr_image_id]" value="<?php echo esc_attr( $qr_image_id ); ?>" />
				...
				<tbody>
					<?php foreach ( $days as $key => $label ) : ?>
						<tr>
							...
							<td>
								<input type="time" name="tsrbsettings[operating_hours][<?php echo esc_attr( $key ); ?>][open]"
									value="<?php echo isset( $operating_hours[ $key ]['open'] ) ? esc_attr( $operating_hours[ $key ]['open'] ) : ''; ?>">
							</td>
							<td>
								<input type="time" name="tsrbsettings[operating_hours][<?php echo esc_attr( $key ); ?>][close]"
				...
				<tbody>
					<?php if ( ! empty( $blocked_dates ) && is_array( $blocked_dates ) ) : ?>
						<?php foreach ( $blocked_dates as $date ) : ?>
							<tr>
								<td>
									<input type="date" name="tsrbsettings[blocked_dates][]" value="<?php echo esc_attr( $date ); ?>" style="width: 100%;">
								</td>
								<td>
		...
		<?php elseif ( 'emails' === $current_tab ) : ?>
			...
			<p class="description">
				...
				<code>{old_start_time}</code>,
				<code>{old_end_time}</code>.
			</p>
			<p class="description">
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking baru (judul email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<input type="text" class="regular-text"
								   name="tsrbsettings[emails][customer_new_subject]"
								   value="<?php echo esc_attr( $emails['customer_new_subject'] ); ?>">
						</td>
					</tr>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking baru (isi email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<?php
							wp_editor(
								...
								array(
									'textarea_name' => 'tsrbsettings[emails][customer_new_body]',
									'media_buttons' => true,
									'teeny'         => false,
								)
							);
					...
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking dikonfirmasi (judul email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<input type="text" class="regular-text"
								   name="tsrbsettings[emails][customer_paid_subject]"
								   value="<?php echo esc_attr( $emails['customer_paid_subject'] ); ?>">
						</td>
					</tr>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking dikonfirmasi (isi email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<?php
							wp_editor(
								...
								array(
									'textarea_name' => 'tsrbsettings[emails][customer_paid_body]',
									'media_buttons' => true,
									'teeny'         => false,
								)
							);
					...
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking dibatalkan (judul email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<input type="text" class="regular-text"
								   name="tsrbsettings[emails][customer_cancel_subject]"
								   value="<?php echo esc_attr( $emails['customer_cancel_subject'] ); ?>">
						</td>
					</tr>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking dibatalkan (isi email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<?php
							wp_editor(
								...
								array(
									'textarea_name' => 'tsrbsettings[emails][customer_cancel_body]',
									'media_buttons' => true,
									'teeny'         => false,
								)
							);
					...
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Admin: notifikasi booking baru (judul email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<input type="text" class="regular-text"
								   name="tsrbsettings[emails][admin_new_subject]"
								   value="<?php echo esc_attr( $emails['admin_new_subject'] ); ?>">
						</td>
					</tr>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Admin: notifikasi booking baru (isi email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<?php
							wp_editor(
								...
								array(
									'textarea_name' => 'tsrbsettings[emails][admin_new_body]',
									'media_buttons' => true,
									'teeny'         => false,
								)
							);
					...
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking di-reschedule (judul email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<input type="text" class="regular-text"
								   name="tsrbsettings[emails][customer_reschedule_subject]"
								   value="<?php echo esc_attr( $emails['customer_reschedule_subject'] ); ?>">
						</td>
					</tr>
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Customer: booking di-reschedule (isi email)', 'tribuna-studio-rent-booking' ); ?>
						</th>
						<td>
							<?php
							wp_editor(
								...
								array(
									'textarea_name' => 'tsrbsettings[emails][customer_reschedule_body]',
									'media_buttons' => true,
									'teeny'         => false,
								)
							);
		...
		<?php elseif ( 'workflow' === $current_tab ) : ?>
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
								   ...
								   name="tsrbsettings[workflow][auto_cancel_unpaid_hours]"
					...
					<tr>
						<th scope="row">
							<label for="tsrb_auto_cancel_unpaid_same_day_hours">
								<?php esc_html_e( 'Auto-cancel booking belum dibayar di hari H setelah (jam)', 'tribuna-studio-rent-booking' ); ?>
							</label>
						</th>
						<td>
							<input type="number" min="0" step="1"
								   id="tsrb_auto_cancel_unpaid_same_day_hours"
								   name="tsrbsettings[workflow][auto_cancel_unpaid_same_day_hours]"
								   value="<?php echo esc_attr( (int) $workflow['auto_cancel_unpaid_same_day_hours'] ); ?>">
							<p class="description">
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
								   ...
								   name="tsrbsettings[workflow][payment_deadline_hours]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
								   ...
								   name="tsrbsettings[workflow][min_lead_time_hours]"
					...
					<tr>
						...
						<td>
							<label>
								<input type="checkbox"
									   name="tsrbsettings[workflow][require_manual_approval]"
					...
					<tr>
						...
						<td>
							<label>
								<input type="checkbox"
									   name="tsrbsettings[workflow][prevent_new_if_pending_payment]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
								   ...
								   name="tsrbsettings[workflow][max_active_bookings_per_user]"
					...
					<tr>
						...
						<td>
							<p>
								<label>
									<input type="checkbox"
										   name="tsrbsettings[workflow][reschedule_admin_only]"
							...
							<p>
								<label>
									<input type="checkbox"
										   name="tsrbsettings[workflow][allow_member_reschedule]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
								   ...
								   name="tsrbsettings[workflow][reschedule_cutoff_hours]"
					...
					<tr>
						...
						<td>
							<p>
								<label>
									<input type="checkbox"
										   name="tsrbsettings[workflow][reschedule_allow_pending]"
					...
					<tr>
						...
						<td>
							<p>
								<label>
									<input type="checkbox"
									       name="tsrbsettings[workflow][allow_member_cancel]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
							       ...
							       name="tsrbsettings[workflow][refund_full_hours_before]"
					...
					<tr>
						<th scope="row">
							<label for="tsrb_refund_partial_hours_before">
								<?php esc_html_e( 'Partial refund jika dibatalkan di hari H sebelum (jam)', 'tribuna-studio-rent-booking' ); ?>
							</label>
						</th>
						<td>
							<input type="number" min="0" step="1"
							       id="tsrb_refund_partial_hours_before"
							       name="tsrbsettings[workflow][refund_partial_hours_before]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" max="100" step="1"
							       ...
							       name="tsrbsettings[workflow][refund_partial_percent]"
					...
					<tr>
						...
						<td>
							<input type="number" min="0" step="1"
							       ...
							       name="tsrbsettings[workflow][refund_no_refund_inside_hours]"
				...
				<tbody>
					<tr>
						...
						<td>
							<label>
								<input type="checkbox"
									   name="tsrbsettings[integrations][google_calendar_enabled]"
					...
					<tr>
						...
						<td>
							<input type="text" class="regular-text"
								   ...
								   name="tsrbsettings[integrations][google_calendar_id]"
					...
					<tr>
						...
						<td>
							<input type="text" class="regular-text"
								   ...
								   name="tsrbsettings[integrations][google_client_id]"
					...
					<tr>
						...
						<td>
							<input type="text" class="regular-text"
								   ...
								   name="tsrbsettings[integrations][google_client_secret]"
					...
					<tr>
						...
						<td>
							<textarea id="tsrb_whatsapp_default_message"
									  name="tsrbsettings[integrations][whatsapp_default_message]"
									  rows="4"
		...
		<?php
		// Tombol global di bawah (opsional).
		submit_button();
		?>
	</form>
</div>
